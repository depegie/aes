...
def aes256_cbc(number : int, one_packet : bool):
...
def aes256_ctr(number : int, one_packet : bool):
    pkts_in = open(STIMULUS_DIR+'pkts_in.txt', 'w')
    pkts_out = open(STIMULUS_DIR+'pkts_out.txt', 'w')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--one-packet", action="store_true", help="Włącza tryb szczegółowy")
    ...
    match module:
        case 'aes256_cbc_iter':
            aes256_cbc(number, one_packet)
        case 'aes256_cbc_comb':
            aes256_cbc(number, one_packet)
        case 'aes256_ctr_iter':
            aes256_ctr(number, one_packet)
